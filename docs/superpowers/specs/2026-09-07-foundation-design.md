...
| Зум и панорама | Камера как отдельная сущность; холст размером с окно |
...
undo/redo; тесты; перевод React-оболочки на новый движок.

**Не входит:** зум и панорама в UI, инструменты фигур, растеризация картинок, растровый
экспорт, сохранение и шаринг, скриншотные эталоны, зашитый эмодзи-шрифт, редизайн.

Пользовательский функционал после итерации — ровно текущий плюс undo/redo: кисть, ластик,
очистка, копирование в текст. Внешний вид сохраняется.
...
В этой итерации камера всегда `{ offsetX: 0, offsetY: 0, zoom: 1 }` — визуально ничего
не меняется. Но координатная арифметика уже вынесена и покрыта тестами, поэтому зум
и панорама позже сводятся к изменению полей камеры.
...
```ts
class GlyphAtlas {
  constructor(cellSizePx: number, dpr: number, fontStack: string)
  get(emoji: Emoji): CanvasImageSource   // растеризует при первом обращении
  clear(): void
}
```
...
Цвета фона, линий сетки, размер клетки, шрифтовой стек, флаг сглаживания. Пиксельный
редизайн будет правкой этого модуля и React-оболочки, не логики рендера.
...
function attachPointerInput(
  el: HTMLElement,
  handlers: { onDown(cell: Cell): void; onMove(cell: Cell): void; onUp(): void },
...
class Editor {
  ...
  getSnapshot(): EditorState      // { brush, toolId, canUndo, canRedo, isEmpty }
...
Внешний вид сохраняется: та же вёрстка, те же классы Tailwind, плюс две новые кнопки.
...
- `operations` — применение, инверсия, обратимость (применить и откатить → исходное
  ...
  касании клетки и возвращает `null` на пустом мазке;
...
- `glyphAtlas` — растеризация даёт непустой буфер; глиф центрирован (проверка по
  распределению непрозрачных пикселей); повторный запрос не растеризует заново;
- `renderScene` — сетка нарисована, клетка со значением отличается от пустой,
  перерисовка после стирания не оставляет следов;
- `pointer` — клик рисует одну клетку, протаскивание рисует связную линию без разрывов,
  выход за границу холста не срывает мазок, слушатели снимаются;
...
- `Editor` — полный цикл: рисование, undo, redo, очистка, `destroy` без утечек.
...
3. **Ядро.** `scene`, `operations`, `history`, `camera`, `line`, `export/text` — через
   тесты вперёд.
4. **Рендер.** `theme`, `glyphAtlas`, `renderScene` — с browser-тестами.
5. **Ввод и инструменты.** `pointer`, `brush`, `eraser`.
6. **Editor и UI.** Фасад, перевод React-оболочки, кнопки undo/redo.
7. **Уборка.** Удаление старого `lib/EmojiCanvas.ts` и осиротевших хуков, обновление
   README и CLAUDE.md.
